...
export const MediaAssetsListPage = () => {
  const { params, ready, missingRequired } = useQueryParams(mediaAssetsListParams);
  ...
  useEffect(() => {
    ...
    fetchMediaAssetList(listId)
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Failed to load media assets list');
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  if (missingRequired) {
    return <ErrorStatus title="Missing list ID" description="The URL must contain ?id=..." />;
  }

  if (loading) {
    return <LoadingStatus title="Loading media assets list" description="Please wait…" />;
  }

  if (error) {
    return <ErrorStatus title="Failed to load media assets list" description={error} />;
  }
