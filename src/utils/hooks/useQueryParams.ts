...
export const useQueryParams = <T extends Record<string, ParamConfig>>(
  ...
  const missingRequired = Object.entries(schema).some(
    ([key, cfg]) =>
      cfg.required && !searchParams.get(key) && !(runtimeConfig.USE_MOCK_DATA && cfg.demoDefault),
  );

  const ready =
    Object.keys(schema).every((key) => searchParams.has(key)) || !runtimeConfig.USE_MOCK_DATA;
